...
export default function LearnFeatureCard({ feature }: LearnFeatureCardProps) {
  return (
    <div className="relative flex flex-col rounded-2xl border border-brand/20 bg-gradient-to-br from-white to-brand-muted/20 p-6 hover:border-brand/50 hover:shadow-lg hover:shadow-brand-glow/8 transition-all duration-300 group">
      ...
      {/* Icon frame helper */}
      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brand-muted text-brand-dark border border-brand/20 group-hover:text-white group-hover:bg-brand transition-all">
        <FeatureIcon name={feature.iconName} />
      </div>

      <h3 className="mt-5 text-base font-bold text-slate-900 tracking-tight group-hover:text-brand-dark transition-colors">
        {feature.title}
      </h3>
