...
export default function TestimonialCard({ testimonial, onClick, onPlayVideo }: TestimonialCardProps) {
  ...
  const CardContent = (
    <>
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-slate-950 flex items-center justify-center border border-white/10">
        ...
        <div
          className="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-slate-950/90 border border-white/15 text-brand group-hover:text-slate-950 group-hover:bg-brand group-hover:border-brand transition-all duration-300 shadow-md"
      ...
      <div className="mt-4 px-2 pb-1.5 flex flex-col justify-end">
        <p className="text-xs font-semibold text-white group-hover:text-brand transition-colors leading-relaxed">
          "{testimonial.title}"
        </p>
  ...
  return (
    <div
      onClick={handleClick}
      className="group relative rounded-2xl border border-white/10 bg-slate-900/60 p-3 hover:border-brand/50 hover:shadow-lg hover:shadow-brand-glow/8 transition-all duration-300 cursor-pointer flex flex-col justify-between"
    >
      {CardContent}
    </div>
  );
