...
export default function TestimonialCard({ testimonial, onClick, onPlayVideo }: TestimonialCardProps) {
  ...
  const CardContent = (
    <>
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-slate-100 flex items-center justify-center border border-slate-200">
        ...
        <div
          className="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-white/95 border border-slate-200 text-brand-dark group-hover:text-slate-950 group-hover:bg-brand group-hover:border-brand transition-all duration-300 shadow-md"
      ...
      <div className="mt-4 px-2 pb-1.5 flex flex-col justify-end">
        <p className="text-xs font-semibold text-slate-900 group-hover:text-brand-dark transition-colors leading-relaxed">
          "{testimonial.title}"
        </p>
        <span className="mt-2 text-[10px] text-slate-500 font-medium">
          {testimonial.clientName}, <span className="text-slate-600">{testimonial.role}</span>
        </span>
      </div>
    </>
  );
  ...
  return (
    <div
      onClick={handleClick}
      className="group relative rounded-2xl border border-brand/20 bg-gradient-to-br from-white to-brand-muted/15 p-3 hover:border-brand/50 hover:shadow-lg hover:shadow-brand-glow/8 transition-all duration-300 cursor-pointer flex flex-col justify-between"
    >
      {CardContent}
    </div>
  );
